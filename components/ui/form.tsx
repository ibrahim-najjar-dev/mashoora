...
type SystemImageProps = React.ReactNode;
...
const Colors = {
  systemGray4: NAV_THEME.dark.destructive, // "rgba(209, 209, 214, 1)",
  secondarySystemGroupedBackground: NAV_THEME.dark.secondary, // "rgba(255, 255, 255, 1)",
};
...
const styles = StyleSheet.create({
  ...
  groupedList: {
    backgroundColor: NAV_THEME.dark.secondary,
    borderTopWidth: 0.5,
...
export function ScrollView(
  ...
  const { top: statusBarInset } = useSafeAreaInsets(); // inset of the status bar
  ...
  return (
    <Animated.ScrollView
      ...
      {...props}
      style={[{ backgroundColor: NAV_THEME.dark.background }, props.style]}
    />
  );
}
...
type FormTextProps = TextProps & {
  ...
  /** Adds a prefix icon component to the left of the text */
...
export function Section({
  ...
  const childrenWithSeparator = allChildren.map((child, index) => {
    ...
    if (resolvedProps.hintBoolean != null) {
      // You can replace these with your own icon components
      resolvedProps.hint ??= resolvedProps.hintBoolean ? (
        <div>✓</div> // Replace with your checkmark icon component
      ) : (
        <div>✗</div> // Replace with your cross icon component
      );
    }
    ...
    >
      ...
      {footer && (
        <RNText
          dynamicTypeRamp="footnote"
          style={{
            color: NAV_THEME.dark.destructive,
            paddingHorizontal: 20,
            paddingTop: 8,
            fontSize: 14,
          }}
...
function LinkChevronIcon({
  ...
  if (systemImage && React.isValidElement(systemImage)) {
    return systemImage;
  }
...
export function HStack(props: ViewProps) {
  ...
  return (
    <View
      {...props}
      style={mergedStyleProp<ViewStyle>(
        styles.hstack,
  ...
  return styleProps
    .map((style) => {
      if (Array.isArray(style)) {
        return mergedStyleProp(...(style as StyleProp<TStyle>[]));
      } else {
        return style;
      }
    })
